...
class Solution():
    ...
    def __init__(self, appliance, timings):
        ...
        """
        on_off_timings = [1 for i in appliance.phases] + [0 for i in range(timings.length - appliance.ScheduleLength)] #Create an array with the energy consupmtion for the appiliance and 0s when its not on
        random.shuffle(on_off_timings)
        self.solutionSchedule = []
        appliancePhaseIndex = 0
        for i in on_off_timings:
            if i == 1:
...
def swap_2_list_indices(list, i, j):
    """
    A function to swap two given list indices in a given list.
    """
    first_swap = list[i]
    list[i] = list[j]
    list[j] = first_swap
...
def small_improvements(solution):
    """
    Swaps a 0 and 1 in the on / off timings to check if the solution improves and if it does then adds that to a list of the new best solutions.
    """
    new_best_costs = []
    current_best_cost = solution.cost
    test_improve_solution = solution
    for i in range(len(solution.onOff)):
        for j in range(len(solution.onOff)):
            swap_2_list_indices(test_improve_solution.onOff, i, j)
            solution.solutionSchedule = []
            appliancePhaseIndex = 0
            for k in test_improve_solution.onOff:
                if k == 1:
                    test_improve_solution.solutionSchedule.append(solution.appliance.phases[appliancePhaseIndex])
                    appliancePhaseIndex += 1
                else:
                    test_improve_solution.solutionSchedule.append(0)
            total = 0
            for i in range(test_improve_solution.length):
                total += test_improve_solution.solutionSchedule[i] * test_improve_solution.timings.costPerPeriod[i]
            if total < current_best_cost:
                new_best_costs = []
                new_best_costs.append(test_improve_solution)
                current_best_cost = total
            if total == current_best_cost:
                new_best_costs.append(test_improve_solution)
            test_improve_solution = solution
    return new_best_costs


testAppliance, testTimings = open_file("p2.txt")
costs, schedules, best_cost = task1(testAppliance, testTimings, 10000000)
...
print(small_improvements(schedules[0]))
